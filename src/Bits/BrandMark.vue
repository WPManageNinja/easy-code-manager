...
/*
 * The FluentSnippets mark: a rounded accent tile with the shield knocked out of it in
 * white, and the </> glyph back in accent inside.
 *
 * Ported from website/src/components/Logo.astro, which is the source of truth - the shield
 * and glyph paths are lifted verbatim from the plugin's official icon on WordPress.org
 * (https://ps.w.org/easy-code-manager/assets/icon.svg) so the admin, the product site and
 * the plugin listing are all showing the same shape. Don't redraw them; if the mark
 * changes it changes there first and is copied here.
 *
 * Tile geometry is measured off FluentCart's own app tile so the two read as one family:
 * corner radius is 0.10 of the tile, and the mark sits in a centred safe box of 0.6714 of
 * it. FluentCart's glyph is wider than tall so its width is what the safe box governs;
 * this shield is taller than wide, so height governs here. Matching the constraining axis
 * is what keeps the padding equal rather than making one mark crowd its tile more.
 *
 * This replaced a PNG wordmark that had to be recoloured in dark mode with
 * `filter: brightness(0) invert(.96)` - which flattened the brand colour to grey and left
 * the mark soft on a HiDPI screen. Drawn rather than filtered, it is sharp at any size and
 * needs no second file for the dark theme: the tile carries its own colour in both.
 */
const SHIELD = 'M86.021 23.2424C100.351 26.9727 105.4 28.4285 105.4 28.4285C105.4 28.4285 106.174 43.2132 99.7139 69.9622C94.4735 91.4208 75.1461 103.369 64.1414 110.171L63.9121 110.313L63.6344 110.141C52.6207 103.333 33.298 91.3894 28.1103 69.9622C21.6051 43.2132 22.3329 28.4285 22.3329 28.4285C22.3329 28.4285 27.337 26.9272 41.6668 23.2424C56.861 19.3302 63.8667 17.9199 63.8667 17.9199C63.8667 17.9199 70.8723 19.3302 86.021 23.2424Z';
...
export default {
    name: 'BrandMark',
    props: {
        /* Rendered size in px; the tile is square. The app bar uses 30. */
        size: {
            type: Number,
            default: 30
        }
...
<template>
    <svg class="fsnip_brand_mark" :width="size" :height="size" viewBox="0 0 128 128"
         xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
        <rect class="fsnip_brand_tile" width="128" height="128" rx="12.8"/>
        <g transform="translate(4.4681 4.4681) scale(0.9302)">
            <path class="fsnip_brand_shield" :d="shield"/>
            <path class="fsnip_brand_glyph" :d="glyph"/>
        </g>
    </svg>
</template>
